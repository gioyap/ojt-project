...
export async function POST(request: Request) {
	...
	const { time_id, time_in, time_out, editcount } = await request.json();
	...
	// Fetch original log with editcount included
	const { data: originalLog, error: originalLogError } = await supabase
		.from("timelogs")
		.select("trainee_id, total_dayhours, date, status_logs, editcount")
		.eq("time_id", time_id)
		.single();
	...
	const currentEditCount = originalLog.editcount || 0;
	const newEditCount =
		editcount !== undefined ? editcount : currentEditCount + 1;
	const MAX_EDITS = 3;

	if (currentEditCount >= MAX_EDITS) {
		return NextResponse.json(
			{ error: "Maximum edit limit (3) reached for this time log" },
			{ status: 400 }
		);
	}
	...
	// Update timelogs with status_logs and editcount
	const { error: timelogError } = await supabase
		...
		.update({
			...
			editcount: newEditCount, // Update editcount
		})
		.eq("time_id", time_id);
	...
	if (internError || !internData) {
		return NextResponse.json(
			{ error: "Error fetching intern data" },
			{ status: 500 }
		);
	}
	...
	return NextResponse.json({
		...
		total_dayhours,
		status_logs: total_dayhours > 0 ? "Present" : "Absent",
		editcount: newEditCount, // Return updated editcount
	});
}
